            ...
            Form {
                ...
                Section(
                    header: VStack(alignment: .leading) {
                        Text("Video Stream")
                            .font(.headline)
                        Text("Configure video input and retention settings.")
                            .font(.caption)
                            .foregroundColor(.gray)
                    }
                ) {
                    // Toggle for enabling/disabling video stream
                    Toggle("Enable Video Stream", isOn: $sessionConfig.video.videoStream)
                        .padding(.vertical, 5)
                    
                    // Picker for maximum resolution
                    VStack(alignment: .leading, spacing: 5) {
                        Text("Max Resolution")
                            .font(.subheadline)
                    ...
                    // Picker for retain video options
                    VStack(alignment: .leading, spacing: 5) {
                        Text("Retain Video")
                            .font(.subheadline)
                        Picker("Retain Video", selection: Binding(
                            get: { sessionConfig.video.retainVideo ?? "message_based" },
                            set: { sessionConfig.video.retainVideo = $0 }
                        )) {
                            Text("Disable").tag("disable")
                            Text("Message Based").tag("message_based")
                            Text("Time Based").tag("time_based")
                        }
                        .pickerStyle(SegmentedPickerStyle())
                    }
                    
                    // Conditionally show settings based on the retainVideo selection
                    if sessionConfig.video.retainVideo != "disable" {
                        if sessionConfig.video.retainVideo == "message_based" {
                            VStack(alignment: .leading, spacing: 5) {
                                Text("Frames Retained per Message")
                                    .font(.subheadline)
                                Stepper(value: $sessionConfig.video.retainPerMessage, in: 1...10) {
                                    Text("\(sessionConfig.video.retainPerMessage) frame(s)")
                                }
                            }
                        } else if sessionConfig.video.retainVideo == "time_based" {
                            VStack(alignment: .leading, spacing: 5) {
                                Text("Seconds per Retained Frame")
                                    .font(.subheadline)
                                Stepper(value: $sessionConfig.video.secondPerRetain, in: 1...60) {
                                    Text("\(sessionConfig.video.secondPerRetain) second(s)")
                                }
                            }
                        }
                        
                        // This control is common to both Message and Time Based modes
                        VStack(alignment: .leading, spacing: 5) {
                            Text("Max Messages with Retained Frame")
                                .font(.subheadline)
                            Stepper(value: $sessionConfig.video.maxMessageWithRetainedVideo, in: 1...20) {
                                Text("\(sessionConfig.video.maxMessageWithRetainedVideo) message(s)")
                            }
                        }
                    }
                }
